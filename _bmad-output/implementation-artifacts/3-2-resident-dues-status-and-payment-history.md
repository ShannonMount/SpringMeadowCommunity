...
- [x] Add resident-safe financial read schema support. (AC: 1, 2, 3)
  - [x] Add the next ordered migration after `supabase/migrations/202605110001_create_assessment_cycles_and_assessments.sql`, likely `supabase/migrations/202605110002_create_payment_records_and_resident_financial_reads.sql`.
  - [x] Create `public.payments`, `public.payment_allocations`, and `public.payment_events` using the canonical payment data model fields, indexes, integer cents, status/method/payer checks, and `community_id` on every HOA-scoped row.
  - [x] Enable RLS on new payment tables. Residents may select only posted property payment records for active linked properties when `property_memberships.can_view_balance = true`; guests must get no authenticated table policy.
  - [x] Add a follow-up resident `select` policy on `public.assessments` for active linked properties with `can_view_balance = true`; keep assessment writes behind the Story 3.1 audited RPCs.
  - [x] Do not broaden `public.assessment_cycles` resident access unless the page truly needs cycle metadata; due dates and descriptions can come from `assessments`.
  - [x] Do not expose `payment_events` to residents. Webhook event storage is for Story 3.6 and admin monitoring later.
- [x] Add a server-only resident dues/payment read service. (AC: 1, 2, 3)
  - [x] Add `server/services/payments/resident-dues.ts` or an equivalently named server-only service in the payments domain.
  - [x] Reuse `getResidentPortalMemberships()` as the source of authenticated profile, active memberships, masked account number, property IDs, community IDs, and permission booleans.
  - [x] Query `properties` summary fields, resident-visible `assessments`, and resident-visible `payments` only for membership-derived property IDs and matching community IDs.
  - [x] Apply `canViewBalance` in the service mapping, not only in UI. If disabled, return `null`/empty financial fields and a permission-aware state for that property.
  - [x] Return typed privacy-safe states for unauthenticated, profile unavailable, no active membership, dues unavailable, and successful dues data.
  - [x] Do not return raw Supabase errors, raw account numbers, owner names, public payment codes, guest emails/phones, Stripe IDs, internal IDs in UI-facing fields, payment event payloads, private documents, or message contents.
- [x] Replace the resident payments placeholder with a real dues/history view. (AC: 1, 2, 3)
  - [x] Update `app/(resident)/portal/(member)/payments/page.tsx`; keep it inside the existing URL-neutral `(member)` route group at `/portal/payments`.
  - [x] Render one property-scoped section/card per active membership so multiple properties never show a combined global balance.
  - [x] For permitted properties, show status, current balance, next due date, open assessment due items, recent posted payment history, and an accessible empty state when no posted payments exist.
  - [x] For `can_view_balance = false`, hide balances, due amounts, assessment line items, and payment history details; show clear resident-facing permission copy.
  - [x] Gate any pay-dues affordance by `canPayDues`, but do not implement Stripe Checkout, Payment Element, pending payment creation, or payment session server actions in this story.
- [x] Extend the dashboard payment summary without weakening privacy. (AC: 1, 2)
  - [x] Update `server/services/auth/resident-dashboard.ts` only if needed to include a compact recent payment signal; preserve its existing scoped property summary behavior.
  - [x] Update `app/(resident)/portal/(member)/page.tsx` to show a compact recent payment/history summary or a clear link to full history for permitted properties.
  - [x] Preserve the Story 2.7 date-only handling pattern for `next_due_date` and any assessment due dates.
  - [x] Keep dashboard data fetching efficient: one active-membership resolution, bounded property/assessment/payment queries, no broad scans, and no N+1 queries for each property.
- [x] Preserve Epic 3 boundaries for later payment stories. (AC: 1, 2, 3)
  - [x] Do not create resident Stripe sessions; Story 3.3 owns that.
  - [x] Do not create guest lookup/session behavior; Stories 3.4 and 3.5 own those.
  - [x] Do not process Stripe webhooks, allocate payments, send receipt emails, or update balances from payments; Stories 3.6 and 3.7 own those.
  - [x] Do not build admin payment records/manual payments; Story 3.8 owns that.
  - [x] Keep any seeded/manual payment rows for tests as static history fixtures only if needed, not a manual recording workflow.
- [x] Extend verification. (AC: 1, 2, 3)
  - [x] Add `tests/resident-dues-status.test.mjs`.
  - [x] Test the payment schema migration, constraints, indexes, RLS enablement, resident assessment read policy, and no resident access to payment events.
  - [x] Test the resident dues service is server-only, reuses `getResidentPortalMemberships()`, scopes by authorized property IDs and community IDs, and maps `canViewBalance` before returning financial fields.
  - [x] Test the payments page renders dues status/history for permitted memberships, permission-aware copy for `can_view_balance = false`, and a polished empty state for no payment history.
  - [x] Test the dashboard preserves existing dues summary behavior and does not leak payment history when balance viewing is disabled.
  - [x] Test privacy exclusions: no raw account number, owner display name, public payment code, guest contact info, Stripe customer/session/payment intent/charge IDs, payment event payload, raw Supabase error, private document, message, service-role, or unrelated property data.
  - [x] Run `npm test`, `npm run typecheck`, `npm run lint`, `npm run build`, and `git diff --check`.

### Review Findings

- [x] [Review][Patch] Resident payment RLS exposes sensitive table columns [supabase/migrations/202605110002_create_payment_records_and_resident_financial_reads.sql:161]
- [x] [Review][Patch] Payment allocations can cross properties within one community [supabase/migrations/202605110002_create_payment_records_and_resident_financial_reads.sql:119]
- [x] [Review][Patch] Payment history limit is global instead of per property [server/services/payments/resident-dues.ts:258]
- [x] [Review][Patch] Payment action is hidden when `can_pay_dues` is true but `can_view_balance` is false [app/(resident)/portal/(member)/payments/page.tsx:301]
- [x] [Review][Patch] Dashboard lacks permission-aware copy when balance viewing is disabled [app/(resident)/portal/(member)/page.tsx:126]
- [x] [Review][Patch] Open dues items are styled as nested cards inside the property card [app/(resident)/portal/(member)/payments/page.tsx:160]
...
- Code review fixes: `npm test`, `npm run typecheck`, `npm run lint`, `npm run build`, and `git diff --check` passed.

### Completion Notes List

- Ultimate context engine analysis completed - comprehensive developer guide created.
- Added resident-safe payment table foundations, allocation/event tables, indexes, trigger helpers, RLS, resident posted payment reads, and resident assessment reads gated by active linked memberships with `can_view_balance`.
- Added the server-only resident dues read service that reuses the resident portal membership resolver, scopes all financial reads by membership-derived property/community IDs, and strips financial fields when `canViewBalance` is false.
- Replaced the resident payments placeholder with property-scoped dues status, open assessment, posted history, permission-hidden, and empty-history states.
- Added a dashboard payment-history entry point gated by `canViewBalance` while preserving the existing property summary data flow and date-only formatting pattern.
- Preserved Epic 3 payment boundaries: no Stripe session creation, guest lookup/session flow, webhook processing, receipt email, payment allocation mutation, or admin manual payment workflow was added.
- Resolved code review findings by restricting resident payment table column privileges, adding a resident-safe history view, enforcing payment allocation property scope, limiting history per property, preserving payment actions for payment-authorized hidden-balance memberships, adding dashboard permission copy, and removing nested card styling from open dues items.
...
- 2026-05-11: Implemented resident dues status and payment history read path; marked ready for review after successful verification.
- 2026-05-11: Addressed code review findings and marked story done.
